# Голосовой ассистент Вокс

Вокс - русский голосовой ассистент для работы оффлайн. Требует Python 3.5+ (зависимость может быть меньше, но в любом случае Python 3)
...
Через сервис [VseGPT.ru](https://vsegpt.ru/), еще один проект автора Вокс: 
...
После установки будут доступны следующие команды: "Вокс привет", "Вокс подбрось монетку", "Вокс подбрось кубик", "Вокс игра больше меньше", "Вокс таймер три минуты"
...
Пример: "Вокс привет", "Вокс подбрось монетку", "Вокс подбрось кубик", "Вокс игра больше меньше", "Вокс таймер три минуты"
...
3. После запуска проверить можно простой командой - скажите "Вокс, привет!" в микрофон

**Папка с настройками options появится после первого запуска Вокса, в ней можно поправить настройки.**
...
```
- Вокс, установи таймер на два с половиной часа
...
Запуск всех команд начинается с имени ассистента (настраивается в options/core.json, по умолчанию - Вокс). 
Так сделано, чтобы исключить неверные срабатывания при постоянном прослушивании микрофона.
Далее будут описываться команды без префикса "Вокс".
...
Автор Вокс не несёт ответственности за их содержание!
...
Есть хороший сторонний плагин, позволяющий запускать сценарии Home Assistant через Вокса:
...
```python
{
    "contextDefaultDuration": 10, # Время в секундах, пока Вокс находится в контексте (контекст используется в непрерывном чате, играх и пр.; в контексте не надо использовать слово Вокс)
    "contextRemoteWaitForCall": false, # должна ли Вокс ждать от клиентов сингнала "Проигрывание ответа закончена, запускаем время для контекста?"
    ...
    "voiceAssNames": "вокс|вокса|воксу", # Если это появится в звуковом потоке, то дальше будет команда. (Различные имена помощника, рекомендуется несколько)
...
Мультиинсталляция в режиме "клиент-сервер" несколько сложнее, но позволяет управлять Воксом:
...
Если вы хотите поддержать мой интерес и сделать так, чтобы Вокс, 
...
- Рассказать кому-то об Воксе, или помочь её настроить.
